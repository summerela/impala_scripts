...
colnames(comp_het) = c("gene_name", "chr", "qual", "filter", "kav_freqPct", "kav_count", "nb_rsID", "nb_pos", "nb_ref", "nb_alt", "m_pos", "m_ref", "m_alt", "f_pos", "f_ref", "f_alt")

#group by gene
library(dplyr)
by_gene = group_by(comp_het, as.factor(comp_het$gene_name))

head(comp_het, 2)

#pull out list of genes returned
gene_list = as.character(unique(comp_het$gene_name))



group by gene name (for each gene)
if more than one nb_pos
and one of them = mom alt pos and mom alt
and one of them = dad alt pos and dad alt
then compound het

YearlyResults <- tapply(LottiaData,LottiaData$Year,glm(Count~zone,family=poisson))

pairs = tapply(comp_het, comp_het$gene_name, )
