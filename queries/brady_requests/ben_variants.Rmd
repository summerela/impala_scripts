...
#get rid of unnecessary columns
hom_ref.df = hom_refs[,(1:13,26:27,54:55)]
...
#find variants where mother, father and newborn have matching alts at same chr/pos, or
#mom and dad alts match and nb call is null
hom_ref.df = hhom_ref.df[which((hom_ref.df$nb_alt == hom_ref.df$m_alt) & (hom_ref.df$nb_alt == hom_ref.df$f_alt) | 
...
#find matching variant positions in parents
...
mie_hets = merge(nb_het, mom_vars, by = "variant_id")
mie_hets = merge(mie_hets, dad_vars, by = "variant_id")

#clean up results
mie_hets = mie_hets[,c(1,3:13,26:27,40:41)]
colnames(mie_hets) = c("variant_id", "qual", "filter", "rsID", "kav_pct", "kav_count",
                       "gene_name", "chr", "pos", "ref", "nb_alt", "nb_gt", "m_alt",
                       "m_gt", "f_alt", "f_gt")

#find variants that are in congruence with MI laws
#if the mother is 0/0, then the nb alt must match the father's alt or be null
mie_hets[(mie_hets$m_gt == "0/0" & mie_hets$f_gt == "0/1" & (mie_hets$nb_alt == mie_hets$f_alt |
        #if the father is 0/0, then the nb alt must match the mother's alt
        mie_hets[(mie_hets$d_gt == "0/0" & mie_hets$m_gt == "0/1" & (mie_hets$nb_alt == mie_hets$m_alt|
        #if both parents are het, nb alt and parent alts must be the same, or nb alt is null
        mie_hets[((mie_hets$m_gt == "0/1" & mie_hets$f_gt== "0/1") &
        #if the mother is 1/1, then the father must be 0/1 or 0/0 and the nb alt must match the mother's alt
        mie_hets[(mie_hets$m_gt == "1/1" & mie_hets$f_gt != "1/1"
        #if the father is 1/1, then the mother must be 0/1 or 0/0 and the nb alt must match the fathers's alt
        mie_hets[(mie_hets$f_gt == "1/1" & mie_hets$m_gt != "1/1"
        mie_hets$nb_alt == "NULL")),],
        mie_hets$nb_alt == "NULL")),],
        ((mie_hets$nb_alt == mie_hets$f_alt)| (mie_hets$nb_alt == "NULL")) & (mie_hets$m_alt == mie_hets$f_alt)),],
        & (mie_hets$nb_alt == mie_hets$m_alt)),],
        & (mie_hets$nb_alt == mie_hets$f_alt)),]

#if nb_het variants are not in no_mie set, then they are MIE
nb_het[grep(paste(nb_het$variant_id, collapse="|"), no_mie$variant_id, invert=TRUE),]
...
#merge together to find intersection of gene:chr:pos
mie_homs = merge(nb_hom_ref, mom_hom_vars, by = "variant_id")
mie_homs = merge(mie_homs, dad_hom_vars, by = "variant_id")

#clean up resuts
mie_homs = mie_homs[,c(1, 3:13,26:27,40:41)]
colnames(mie_homs) = c("variant_id", "qual", "filter", "rsID", "kav_pct", "kav_count",
                       "gene_name", "chr", "pos", "ref", "nb_alt", "nb_gt", "m_alt",
                       "m_gt", "f_alt", "f_gt")

#find variants that are in congruence with MI laws
no_mie_homs = rbind(
  #if the mother is 0/1, then the dad must be 0/0 and nb ref must match
