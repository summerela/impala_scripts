...
query = sqlFetch(conn, "users_selasady.ben_query")
...
For each of the five homozygous alt newborn variants found, a query was run to manually check for all variants at that chromosome and position in the ITMI 102 puzzle table. 

```{r test_hom_alt}

#Mother is 1/1 and father is 0/1
hom_alt_nb[1,]
sqlQuery(conn, "SELECT p.chr, p.pos, p.id, p.ref, p.alt, p.gt, p.sample_id 
         FROM p7_ptb.itmi_102_puzzle p
         WHERE p.chr = '16'
         AND p.pos = 82068897
         AND (p.sample_id LIKE '%01' OR p.sample_id LIKE '%02')", as.is=TRUE)
...
#father 0/1, mother 1/1, this shows up in results as expected
hom_alt_nb[4,]
sqlQuery(conn, "SELECT p.chr, p.pos, p.id, p.ref, p.alt, p.gt, p.sample_id 
         FROM p7_ptb.itmi_102_puzzle p
         WHERE p.chr = '18'
...
The results are consistent with manual examination of the ITMI 102 data due to the exclusion of homozygous parent variants. 
...
#randomly sample 5 hom_ref nb variants
hom_ref_sample = hom_ref_nb[sample(1:nrow(hom_ref_nb), 5, replace=FALSE),]

#no variant calls from parents
hom_ref_sample[1,]
sqlQuery(conn, "SELECT p.chr, p.pos, p.id, p.ref, p.alt, p.gt, p.sample_id 
         FROM p7_ptb.itmi_102_puzzle p
         WHERE p.chr = '3'
...
#both parents 0/0 at this location
hom_ref_sample[3,]
sqlQuery(conn, "SELECT p.chr, p.pos, p.id, p.ref, p.alt, p.gt, p.sample_id 
         FROM p7_ptb.itmi_102_puzzle p
         WHERE p.chr = '11'
...
#only one parent variant reported
hom_ref_sample[4,]
sqlQuery(conn, "SELECT p.chr, p.pos, p.id, p.ref, p.alt, p.gt, p.sample_id 
         FROM p7_ptb.itmi_102_puzzle p
         WHERE p.chr = '2'
...
The results are consistent with data found in the puzzle table. We may want to add in a way of handling newborn variant calls that are missing calls from either/both parents. 
...
```{r test_comp_het}
#group het nb by gene and examine variants on each gene
...
```
Variants found were consistent with manual search conducted on each gene in impala. 
...
For newborn variants genotyped as homozygous ref (0/0):  
...
comp_het.df$variant_type = "comp_het"

homs = rbind(hom_alt.df, hom_ref.df)
write.table(homs, file="./queries//brady_requests/homs.tsv", sep="\t")
close(conn)
