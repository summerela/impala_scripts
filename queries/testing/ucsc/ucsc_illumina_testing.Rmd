...
```sql
SELECT vcf.sample_id as vcf_sample_id, vcf.chromosome as vcf_chrom, vcf.position as vcf_pos,vcf.ref as vcf_ref, vcf.alt as vcf_alt, vcf.id as vcf_rsID, ucsc.name as ucsc_id, 
SUBSTR(ucsc.chrom,4) as ucsc_chrom, ucsc.strand as ucsc_strand, ucsc.txstart as ucsc_txstart, 
...
CASE 
WHEN vcf.position BETWEEN ucsc.cdsstart AND ucsc.cdsend 
THEN 'CDS' 
WHEN vcf.position BETWEEN ucsc.txstart AND ucsc.txend
THEN 'transcript'
             END AS ucsc_feature
FROM p7dev.illumina_test as vcf, public_hg19.ucsc as ucsc
WHERE vcf.filter = "PASS"  
