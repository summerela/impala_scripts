...
class run_admix(object):
    ...
    def subset_vcf(self, input_vcf):
        subset_cmd = r'''nohup {vcftools} --gzvcf {vcfdir}/{vcf} --positions {snps} --recode --stdout | awk
        ' BEGIN {{OFS = "\t"}}  /^##/ {{next}} /!^#CHROM/ {{sub(/^#/,"",$1)}} {{$1=$1; print}} ' - | awk 'NR > 0
        {{$6=".";$7=".";$8=".";$9="GT";print}}' | cut -f10 | cut -d ":" -f 1 | column -t | gzip > {out_dir}/{out_file}_trimmed.vcf.gz'''.format\
...
# admix.make_ref_snps(admix.ref_dir)
    ...
    #     subset_bed_cmd = "nohup {} --bfile {}  --make-bed --out {}/{}_final".format(self.plink_path,input_vcf, snps, self.filtered_out, vcf_base)
    #     print subset_bed_cmd
>>>>>>> a59effa2ac69c3e8371152ccdc0d04221b973e09
