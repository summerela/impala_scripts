...
class VariantAnnotator(object):
  ...
  def classify_feature(self, gene, ref_start, ref_end, ref_nuc, var_nuc):
    ...
    #Prachi - make ref_end to be > ref_start if they are equal for traversing interval tree
    ref_end_find = ref_end if ref_end > ref_start else ref_start+1
    for part in gene_parts.find_values(ref_start, ref_end_find):
    ...
    if len(intersect)==1 and len(intersect['CDS'])==1:
      ...
      evidence.append([parts,gene,'mRNA=%s:protein=%s:exon=%s:strand=%s' % (gene.mRNA,'','',''),True,'NON-SYNONYMOUS',mut_type,ref_nuc,var_nuc,'',''])
    elif mut_type:
      evidence.append([parts,gene,'mRNA=%s:protein=%s:exon=%s:strand=%s' % (gene.mRNA,'','',''),True,'PREDICTED-DISRUPT-TRANSCRIPT',mut_type,ref_nuc,var_nuc,'',''])
...
#Modified to output transcript information for intronic changes - Prachi 03/15/13
      evidence.append([parts,gene,'mRNA=%s:protein=%s:exon=%s:strand=%s' % (gene.mRNA,'','',''),False,'UNKNOWN-INTRONIC',mut_type,ref_nuc,var_nuc,'',''])
    else:
  ...
  def classify_exonic_variant(self, gene, gene_parts, cds, ref_start, ref_end, ref_nuc, var_nuc):
    ...
    #Prachi - handling deletions represented as ref->.
    var_nuc = '' if var_nuc=='.' else var_nuc
    ...
    #Prachi 05/29 - AA position calculated based on relative pos in cDNA derived from chr pos
    ...
    if not ref_aa and not var_aa:
      ...
      result[-1] += ':cdna=%d' % (ref_var_start+1)
      result += [ppc_flag,'SYNONYMOUS',mut_type,ref_cds_nuc,var_cds_nuc,str(ref_aa),str(ref_aa)]
      return result
    ...
    result[-1] += ':cdna=%d' % (ref_var_start+1)
    if fs_flag:
    	result     += [True,'NON-SYNONYMOUS',mut_type,ref_cds_nuc,var_cds_nuc,'','']
    else:
